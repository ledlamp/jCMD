		...
		.then(function (m) {
			...
			if (m) {
				client.binds.set(msg.author.id, {
					...
					traces,
					times: 0,
					timer: setTimeout(function () {client.binds.delete(msg.author.id)}, client.config.maxEditTime) // This removes itself from the map to limit the max time between the command execution and the next command edit / deletion
				})
			}
		})

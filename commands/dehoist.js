...
module.exports = {
	run: async function (msg, p) {
		let renick = p.join(' '), proms = [], su = 0, fa = 0
		...
		return Promise.all(msg.guild.members.map(function (member) {
			...
			if (msg.guild.me.highestRole.comparePositionTo(member.highestRole) > 0 && hoistregex.test(l)) return member.setNickname(renick, `Dehoist - Requested by ${msg.author.tag}`)

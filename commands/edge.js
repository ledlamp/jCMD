		...
		fetch(image.url)
			.then(res => res.body.pipe(new PNG({ filterType: 4 }))
				.on('parsed', function () {
					...
					client.util.done(msg, 'Edge\'d!', new Discord.Attachment(this.pack(), 'demonisedOutput.png'))
				})
			)
	},
	cat: 'img',
...
ote that there are no plans to add support for other image formats, such as JPG or GIF.`
}
function deepHelp(client, msg, obj, p, his) {
	if (obj.run) {
		...
		fields[x] = {
			name: "Cooldown", value: `${toSecs} second${toSecs == 1 ? '' : "s"}`}
			x++
		}
		if (obj.perm) {
			fields[x] = {name: "User permissions", value: client.q.permName(obj.perm)}
			x++
		}
		if (obj.botPerm) {
			fields[x] = {name: "Bot permissions", value: client.q.permName(obj.botPerm)}
			x++
		}
...
exports.run = async function(client, msg, cmd) {
	...
	if (!cmd[0] || !client.commands.get(cmd[0])) client.q.cmdd(msg, client.q.mkEmbed(`❯ Prefix: ${pr ? pr : '(none in DMs)'} - Help`, "❯ Available commands, by category:", client.helpFields))
		...
		let c = cmd.shift()
		deepHelp(client, msg, client.commands.get(c), cmd, c)
	}
}
exports.cat = "info"
exports.cd = 3000

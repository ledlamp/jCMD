...
module.exports = {
	run: async function (msg, p) {
		let text = p.join(' '), ms = undefined
		client.util.done(msg, 'Alright, I got the text. Now tell me what methods you want to apply to it. Keep in mind that you can see all available methods using the `help` command, and you only have 20 seconds.').then(function (mes) {ms = mes})
		return new Promise(function (res, rej) {
			let f = function (message) {
				if (message.channel.id === msg.channel.id && message.author.id === msg.author.id) {
					...
					if (ms) ms.delete().catch(()=>undefined)
					res({content: text})
				}
			}
